...
def find_choice_mappings(raw):
    """
    Recursively search the raw Airtable response for select/multiSelect
    choice definitions. Airtable stores these in various places depending
    on the response format version:
      - columns[].typeOptions.choices (keyed by choice ID)
      - columns[].typeOptions.choiceOrder (array of IDs)
      - data.table.columns[].typeOptions.choices
      - choicesBySelectColumnId (top-level or nested)
      - viewData.choices
    Returns a flat dict: { "selXXXXX": "Human Label", ... }
    """
    choice_map = {}

    def extract_choices_from_obj(obj, depth=0):
        """Walk any dict/list looking for choice-shaped data."""
        if depth > 10:
            return
        if isinstance(obj, dict):
            # Pattern 1: {"id": "selXXX", "name": "Conference"} inside a list
            if "id" in obj and "name" in obj:
                oid = obj["id"]
                if isinstance(oid, str) and oid.startswith("sel"):
                    choice_map[oid] = obj["name"]

            # Pattern 2: typeOptions.choices = {"selXXX": {"name": "Conference"}}
            choices = obj.get("choices")
            if isinstance(choices, dict):
                for k, v in choices.items():
                    if isinstance(k, str) and k.startswith("sel") and isinstance(v, dict):
                        choice_map[k] = v.get("name", v.get("label", k))
            elif isinstance(choices, list):
                for item in choices:
                    extract_choices_from_obj(item, depth + 1)

            # Pattern 3: choicesBySelectColumnId = {"fldXXX": {"selXXX": {...}}}
            cbs = obj.get("choicesBySelectColumnId")
            if isinstance(cbs, dict):
                for col_id, col_choices in cbs.items():
                    if isinstance(col_choices, dict):
                        for k, v in col_choices.items():
            ...
            for key, val in obj.items():
                if key in ("rows", "records"):
                    # Skip row data to avoid false positives
                    continue
                extract_choices_from_obj(val, depth + 1)

        elif isinstance(obj, list):
            for item in obj:
                extract_choices_from_obj(item, depth + 1)

    extract_choices_from_obj(raw)
    return choice_map


def resolve_select_values(value, choice_map):
    """
    Replace select option IDs with human-readable names.
    Handles single select (string) and multiSelect (list of strings).
    """
    if isinstance(value, str) and value.startswith("sel") and value in choice_map:
        return choice_map[value]
    elif isinstance(value, list):
        return [
            choice_map.get(v, v) if isinstance(v, str) and v.startswith("sel") else v
            for v in value
        ]
    return value
...
def parse_airtable_data(raw):
    ...
    resolving select/multiSelect IDs to human-readable labels.
    """
    ...
    # --- Discover choice labels ---
    print("\n  Searching for select option labels...")
    choice_map = find_choice_mappings(raw)
    if choice_map:
        print(f"  Found {len(choice_map)} select option labels:")
        for sid, name in sorted(choice_map.items(), key=lambda x: x[1]):
            print(f"    {sid} -> {name}")
    else:
        print("  ⚠️  No choice labels found in response structure.")
        print("  Will dump raw response to aisafety_raw_debug.json for inspection.")
        with open("aisafety_raw_debug.json", "w", encoding="utf-8") as f:
            json.dump(raw, f, indent=2, ensure_ascii=False, default=str)
        print("  Run: jq 'paths | select(.[-1] == \"choices\")' aisafety_raw_debug.json")
    ...
    for col in columns:
        ...
        col_map[cid] = cname
        col_types[cid] = ctype
        if ctype in ("select", "multiSelect", "singleSelect"):
            select_columns.add(cid)

    print(f"\n  Found {len(col_map)} columns:")
    for cid, cname in col_map.items():
        suffix = " (select)" if cid in select_columns else ""
        print(f"    {cname:35s} {col_types.get(cid, '?')}{suffix}")
    ...
    # --- Convert to clean records, resolving select IDs ---
    records = []
    unresolved_ids = set()
    for row in rows:
        ...
        for col_id, value in cells.items():
            ...
            if col_id in select_columns and choice_map:
                resolved = resolve_select_values(value, choice_map)
                # Track any IDs we couldn't resolve
                if isinstance(value, list):
                    for v in value:
                        if isinstance(v, str) and v.startswith("sel") and v not in choice_map:
                            unresolved_ids.add(v)
                record[col_name] = resolved
            else:
    ...
    if unresolved_ids:
        print(f"\n  ⚠️  {len(unresolved_ids)} unresolved select IDs (labels not in response):")
...
def main():
    ...
    if isinstance(parsed, dict) and "records" in parsed:
        ...
        # Show choice label summary
        choice_labels = parsed.get("choice_labels", {})
        if choice_labels:
            print(f"\n{'=' * 60}")
            print(f"Resolved {len(choice_labels)} select option labels")
            print("=" * 60)
        else:
            print(f"\n⚠️  No choice labels resolved.")
            print("  Inspect aisafety_raw.json to find where Airtable stores them.")
            print("  Try: python3 -c \"")
            print("    import json")
            print("    raw = json.load(open('aisafety_raw.json'))")
            print("    def find(obj, depth=0):")
            print("      if depth > 8: return")
            print("      if isinstance(obj, dict):")
            print("        for k,v in obj.items():")
            print("          if 'choice' in k.lower() or 'select' in k.lower():")
            print("            print(f'  {k}: {str(v)[:200]}')")
            print("          find(v, depth+1)")
            print("      elif isinstance(obj, list):")
            print("        for i in obj[:5]: find(i, depth+1)")
            print("    find(raw)")
            print("  \"")
